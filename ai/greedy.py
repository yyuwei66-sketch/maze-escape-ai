...
def _load_map_txt(path: str) -> Tuple[List[List[int]], List[Pos], List[Pos]]:
    ...
    for i in range(1, height + 1):
        ...
        if len(row) != width:
            raise ValueError(
                f"row {i} width mismatch: expected {width}, got {len(row)}"
            )
    ...
    for line in lines[height + 1:]:
        ...
        if len(values) != 2:
            raise ValueError(
                f"spawn line must contain exactly two integers: {line}"
            )
...
def move_monsters_in_map(player_pos=None, map_path=None, steps_per_turn: int = 2,
                         allow_stay: bool = False,
                         avoid_stacking: bool = True) -> List[List[Pos]]:
    path = _find_map_file(map_path)
    if path is None:
        raise FileNotFoundError(
            "generated_map.txt not found -- run genetic_map.py first.")
    ...
    ctrl = make_greedy_controller(walls, width, height,
                                  ...
                                  allow_stay=allow_stay,
                                  avoid_stacking=avoid_stacking)
    player = player_pos if player_pos is not None else spawns[0]
    monsters = spawns[1:]
    paths = ctrl.decide(player, monsters)

    moved_monsters = [monster_path[-1] for monster_path in paths]
    _write_map_txt(path, grid, [player, *moved_monsters])  # 写终点
    return paths  
