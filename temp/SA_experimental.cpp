...
bool findPathDfs(const Point& now,const Point& target,bool visited[MAP_SIZE][MAP_SIZE],vector<Point>& path)
{
    ...
    for(const Point& nxt:candidates)
    {
        ...
        if(findPathDfs(nxt,target,visited,path))return true;
...
vector<Point> extendPath(vector<Point> path,const Point& target)
{
    ...
    if(findPathDfs(path.back(),target,visited,path))return path;
...
vector<Point> makeInitialPath()
{
    ...
    if(findPathDfs(start,target,visited,path))return path;
