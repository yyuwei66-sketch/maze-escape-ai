            ...
            "monsters": [
                {
                    **pos_to_json(pos),
                    "path": [
                        pos_to_json(p)
                        for p in (self.last_monster_paths[i][1:] if i < len(self.last_monster_paths) else [])
                    ],
                }
                for i, pos in enumerate(self.monsters)
            ],
            "monster_states": [
...
def planned_monster_paths(game: GameState) -> List[List[Pos]]:
    ...
    if ai_name == "sa":
        _, path_points = run_cpp_map_algorithm(
            "sa",
            game.grid,
            game.human,
            game.monsters[0],
        )
        return [[game.monsters[0], *path_points]]
