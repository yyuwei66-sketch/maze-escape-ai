...
The first check is the map format. The game uses a fixed $30 \times 30$ grid, so the generated file must contain 30 rows and 30 columns. Each cell must also be either \texttt{0} or \texttt{1}. In our map file, \texttt{0} represents a walkable tile, and \texttt{1} represents a wall. This check is important because the backend movement logic and the frontend canvas rendering both depend on this fixed format.
